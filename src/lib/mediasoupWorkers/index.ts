...
export type IStartMediasoupWorkers = () => Promise<Array<mediasoup.types.Worker>>;
  ...
  async () => {
    ...
    for (let i = 0; i < config.mediasoup.numWorkers; i++) {
      ...
      if (process.env.MEDIASOUP_USE_WEBRTC_SERVER !== 'false') {
        ...
        // for each Worker.
        const webRtcServerOptions = config.mediasoup.webRtcServerOptions as mediasoup.types.WebRtcServerOptions<mediasoup.types.AppData>;
        const portIncrement = workers.length - 1;

        for (const listenInfo of webRtcServerOptions.listenInfos) {
          listenInfo.port = listenInfo.port! + portIncrement;
        }
